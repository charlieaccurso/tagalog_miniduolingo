"""
Two major updates to make:

1. Define a separate Question answer cleaning function.

2. Choose five random questions from csv files.
"""
import pandas as pd
import time
import random
...
class Question:
    ...
    def clean_string(self, my_string):
        # removes trailing whitespace, replaces uppercase letters, and
        # removes punctuation
    ...
    def get_answer(self):
        ...
        user_answer= self.clean_string(user_answer)
        return user_answer
...
class FillBlank(Question):
    ...
    def check_answer(self, user_answer):
        correct_answer= self.clean_string(self.answer)
...
class Translate(Question):
    ...
    def check_answer(self, user_answer):
        if self.language == 'tl':
            correct_answer= self.clean_string(self.question_en)
            if user_answer == correct_answer:
                self.correct_message()
            else:
                self.incorrect_message()
            ...
            correct_answer= self.clean_string(self.question_tl)
            if user_answer == correct_answer:
                self.correct_message()
            else:
                self.incorrect_message()
...
class FillBlankGenerator:
    ...
    def generate_fillblank(self):
        ...
        for n in range(5):
            i= random.randrange(file_length)
